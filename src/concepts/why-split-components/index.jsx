  ...
  return (
    <section>
      <header className="lesson-header">
        ...
        <h2>한 컴포넌트에 다 넣으면 안 돼? — 컴포넌트를 나누는 이유</h2>
        <p>HTML처럼 한 컴포넌트에 다 때려넣을 수도 있다. 그런데 왜 굳이 나눌까? 이유는 셋 — ① 똑같은 걸 여러 번(재사용), ② 큰 걸 역할별로 나눠 보기 좋게(정리), ③ 바뀐 곳만 다시 그려 빠르게(성능).</p>
      </header>

      <div className="lesson-goal">
        <span className="lesson-goal-tag">🎯 학습 포인트</span>
        <p>
          한 컴포넌트에 다 넣어도 <b>동작은 한다.</b> 하지만 화면이 커지면 <b>세 가지</b> 이유로 나눈다 —
          <b>재사용</b>(같은 걸 여러 곳에), <b>정리</b>(역할별로 나눠 보기 좋게), <b>성능</b>(바뀐 곳만 다시 그리기). 한 파일에 컴포넌트가 여러 개면 보통 <b>정리</b>다.
        </p>
      </div>

      <div className="concept">
        <p className="concept-lead">
          처음엔 이렇게 생각하기 쉽다 — <b>"HTML은 한 파일에 다 적잖아. 컴포넌트도 하나에 다 넣으면 되지, 왜 여러 개로 나눠?"</b>
          맞다, 하나에 다 넣어도 <b>화면은 잘 나온다.</b> 그런데 화면이 커질수록 그 하나가 <b>읽기·고치기 힘들어져서</b>, 아래 <b>세 가지</b> 이유로 나눈다.
          (한 파일에 컴포넌트가 여러 개 보여서 헷갈리는 것도 여기서 온다.)
        </p>
      </div>
...
function ProfilePosts() {
  return (
    <div style={{ padding: 16 }}>
      ...
      {/* 3️⃣ 성능 */}
      <h3 className="section-title">3️⃣ 성능 — 바뀐 조각만 다시 그리기</h3>
      <span className="learn-tag">📎 학습 포인트 · 나눠 두면 화면이 바뀔 때 '바뀐 조각만' 다시 그려서 빠르다 (리렌더링은 3단계에서)</span>
      <p className="section-desc">
        리액트는 값이 바뀌면 그 화면을 <b>다시 그린다 — '리렌더링'</b>이라고 한다. 한 덩어리면 작은 것 하나만 바뀌어도 <b>전체를 통째로 다시</b> 그린다.
        역할별로 나눠 두면 <b>바뀐 조각만</b> 다시 그려서 더 빠르다. 화면이 크고 자주 바뀔수록 이 차이가 커진다.
        (리렌더링은 아직 안 배웠으니 지금은 <b>"나누면 성능에도 도움"</b>만 알아두면 된다 — 자세한 건 <b>3단계 · 리렌더링</b>에서 배운다.)
      </p>
      ...
      <div className="try-it">
        <h4>💡 한 줄 정리</h4>
        <ul>
          <li>나누는 이유 = <b>재사용</b>(같은 걸 여러 곳) · <b>정리</b>(역할별로 나눠 보기 좋게) · <b>성능</b>(바뀐 곳만 다시 그리기).</li>
