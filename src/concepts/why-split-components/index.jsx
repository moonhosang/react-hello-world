  ...
  return (
    <section>
      <header className="lesson-header">
        ...
        <p>HTML처럼 한 컴포넌트에 다 때려넣을 수도 있다. 그런데 왜 굳이 나눌까? 이유는 둘 — ① 똑같은 걸 여러 번(재사용), ② 큰 걸 역할별로 나눠 보기 좋게(정리).</p>
      </header>

      <div className="lesson-goal">
        <span className="lesson-goal-tag">🎯 학습 포인트</span>
        <p>
          한 컴포넌트에 다 넣어도 <b>동작은 한다.</b> 하지만 화면이 커지면 <b>두 가지</b> 이유로 나눈다 —
          <b>재사용</b>(같은 걸 여러 곳에)과 <b>정리</b>(역할별로 나눠 보기 좋게). 한 파일에 컴포넌트가 여러 개면 보통 <b>정리</b>다.
        </p>
      </div>
      ...
      <div className="concept">
        <p className="concept-lead">
          ...
          맞다, 하나에 다 넣어도 <b>화면은 잘 나온다.</b> 그런데 화면이 커질수록 그 하나가 <b>읽기·고치기 힘들어져서</b>, 아래 <b>두 가지</b> 이유로 나눈다.
...
function ProfilePosts() {
  return (
    <div style={{ padding: 16 }}>
      ...
      {/* 심화 보너스 — 성능(리렌더) */}
      <div className="concept">
        <p className="concept-lead">➕ 한 가지 더 (심화) — 성능: '바뀐 조각만' 다시 그리기</p>
        <p className="section-desc" style={{ marginTop: 0 }}>
          리액트는 값이 바뀌면 화면을 <b>다시 그린다(리렌더링)</b>. 한 덩어리면 작은 것 하나만 바뀌어도 <b>전체를 다시</b> 그린다.
          역할별로 나눠 두면 <b>바뀐 조각만</b> 다시 그려서 더 빠르다. (리렌더링은 아직 안 배웠으니 지금은 <b>"나누면 성능에도 도움"</b>만 알아두자 — 자세한 건 <b>3단계 · 리렌더링</b>에서.)
        </p>
      </div>
