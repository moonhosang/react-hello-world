...
export default function Step10Reducer({ onGo }) {
  return (
    <section>
      ...
      {/* 외부 의존성 '왜 골치아픈가' 상세는 도구 레슨 JS 8로 분리 */}
      <p className="section-desc">
        그런데 "라이브러리 = <b>외부 의존성</b>"이 정확히 <b>왜 골치아픈지</b>(전이 의존성·버전 충돌·번들·공급망)는
        상태관리만의 얘기가 아니라 <b>모든 패키지</b>에 해당하는 도구·기초 주제라, <b>JS 8 · 패키지와 의존성</b>으로 따로 뺐다.
        MUI 전이 의존성 트리·<code>left-pad</code>·"같은 패키지 다른 버전 충돌" 같은 구체 예가 거기 있다.
      </p>
      <TechTags items={[{ label: '🟨 JS 8 · 패키지와 의존성 (자세히)', to: 'js-deps' }]} onGo={onGo} />
