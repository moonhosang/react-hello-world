// ✅ 조립 완성본 — 로직(useSignupForm)은 그대로 두고, 조각(Field·ChipSelect)을 배치하고
// value·onChange·error(관심분야는 selected·onToggle) props로 연결하기만 했다.
...
export default function SolutionAssembly() {
  ...
  return (
    <form className="demo-card" onSubmit={f.handleSubmit}>
      ...
      <Field label="경력" name="level" type="select" options={LEVEL_OPTIONS} value={f.form.level} onChange={f.handleChange} error={f.shownError('level')} />
